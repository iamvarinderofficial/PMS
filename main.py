...
def save_password(username, hashed_password, system, date):
    sample_line = [username, str(hashed_password), system, str(date)]
...
def create_password():
    ...
    # save password
    save_password(username, hashed_password, system, str(datetime.datetime.now()))
...
def login():
    username = request.get_json()['username']
    password = request.get_json()['password']
    usr = config('env_username')
    pwd = config('env_username')
...
def generate_password():
    ...
    # save password
    save_password(username, hashed_password,system, str(datetime.datetime.now()))
    return jsonify({"message ": "The password is successfully generated and saved",
...
def renew():
    ...
    with open("db_file.txt", "w") as f:
        for line in lines:
            if line.split(' ')[0] != username and line.split(' ')[3] != system:
                f.write(line)
        date = datetime.datetime.now()
        sample_line = [username, str(hash), system, str(date)]
        f.write(' '.join(sample_line))
        f.write('\n')
        f.close()
    return jsonify("password update successfully"), 200
